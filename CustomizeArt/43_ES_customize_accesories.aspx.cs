...
{
    public partial class _43_ES_customize_accesories: System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(productId))
                {
                    ...
                    {
                        string script = "Swal.fire('¡Inicia sesión!', 'Necesitas tener una sesión iniciada para añadir un producto al carrito', 'info').then(function() { window.location.href = '01_ES_Login.aspx'; });";
        ...
        public static DataTable ObtenerDetallesDelProducto(int productId)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                MySqlCommand comando = new MySqlCommand($"SELECT Product, Image, Descripcion, Price, Quantity FROM products WHERE ID = {productId}", conn);
        ...
        protected void Unnamed1_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(rawId) && int.TryParse(rawId, out productId))
            {
                ...
                if (CusImage.FileName == "" && impresion.SelectedValue == "Selecciona la cara")
                {
                    // Sin personalizar
                    if (DropDownList1.SelectedValue == "Selecciona la talla" || colorRadioButtonList.SelectedValue == "")
                    {
                        string script = "Swal.fire('OOPS', 'Por favor seleccione una talla y un color', 'error')";
                    ...
                    {
                        string script = "Swal.fire('OOPS', 'No fue posible personalizar el producto', 'error');";
                        ScriptManager.RegisterStartupScript(this, GetType(), "showalert", script, true);
                    }
                ...
                {
                    // Con personalizar
                    if (DropDownList1.SelectedValue == "Selecciona la talla" || colorRadioButtonList.SelectedValue == "" || CusImage.FileName == "" || impresion.SelectedValue == "Selecciona la cara")
                    {
                        string script = "Swal.fire('OOPS', 'Por favor seleccione talla, color, imagen y cara', 'error')";
                        ScriptManager.RegisterStartupScript(this, GetType(), "showalert", script, true);
                        return;
                    }
                    string strFileName;
            ...
            {
                string script = "Swal.fire('OOPS', 'ID del artículo inválido', 'error').then(function() { window.location.href = 'Default_Spanish.aspx'; });";
                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", script, true);
            }
        }
    }
