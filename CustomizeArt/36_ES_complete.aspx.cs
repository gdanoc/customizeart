...
{
    public partial class _36_ES_complete : System.Web.UI.Page
    {
        ...
        protected async void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                ShoppingCartUrl = ResolveUrl("~/07_ES_ShoppingCart.aspx");
                ...
                try
                {
                    ...
                    using (var client = new HttpClient())
                    {
                        ...
                        if (status)
                        {
                            ...
                            Debug.WriteLine($"Respuesta de PayPal: {jsonRespuesta}");
                            ...
                            if (int.TryParse(HttpContext.Current.Session["userID"]?.ToString(), out int userId))
                            {
                                ...
                                if (checkedOut != 0)
                                {
                                    string script = "Swal.fire('Éxito', 'Producto comprado exitosamente', 'success');";
                                    alerta.Text = $"<script>{script}</script>";
                                }
                                else
                                {
                                    string script = "Swal.fire('Oops', 'Algo salió mal con la compra', 'error');";
                                    alerta.Text = $"<script>{script}</script>";
                                }
                            }
                            else
                            {
                                string script = $"Swal.fire('Oops', 'ID de usuario no válido', 'error').then(function() {{ window.location.href = '03_Clothes.aspx'; }});";
                                alerta.Text = $"<script>{script}</script>";
                            }
                        }
                        else
                        {
                            ...
                            string script = $"Swal.fire('Oops', 'Algo salió mal con PayPal: {error}', 'error').then(function() {{ window.location.href = '03_Clothes.aspx'; }});";
                            alerta.Text = $"<script>{script}</script>";
                        }
                    }
                ...
                {
                    string script = $"Swal.fire('Oops', 'Ocurrió un error: {ex.Message}', 'error').then(function() {{ window.location.href = '03_Clothes.aspx'; }});";
                    alerta.Text = $"<script>{script}</script>";
                }
            }
        }
