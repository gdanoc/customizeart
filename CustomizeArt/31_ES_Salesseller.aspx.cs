...
{
    public partial class _31_ES_Salesseller : System.Web.UI.Page
    {
        ...
        protected void Sale_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                btnWaiting.Text = "Esperando"; // Texto en español
                btnWaiting.CssClass = "btn btn-primary btn-sm vertical-button solid-button-yellow"; // Clase CSS con el color actualizado
                btnWaiting.CommandArgument = e.Row.Cells[0].Text; // Asignar el valor de Id a CommandArgument
                ...
                btnReady.Text = "Listo para recoger"; // Texto en español
                btnReady.CssClass = "btn btn-success btn-sm vertical-button solid-button-blue"; // Clase CSS con el color actualizado
                btnReady.CommandArgument = e.Row.Cells[0].Text; // Asignar el valor de Id a CommandArgument
                ...
                btnArrived.Text = "Llegado"; // Texto en español
                btnArrived.CssClass = "btn btn-info btn-sm vertical-button solid-button-green"; // Clase CSS con el color actualizado
                btnArrived.CommandArgument = e.Row.Cells[0].Text; // Asignar el valor de Id a CommandArgument
                ...
                // Agregar la tabla de botones a la fila de la tabla principal
        ...
        protected void btnWaiting_Click(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(btn.CommandArgument, out saleId))
            {
                ...
                if (Result != 0)
                {
                    ...
                    string script = "Swal.fire('Éxito', 'Estado de venta actualizado correctamente', 'success').then(function() { window.location.href = '31_ES_Salesseller.aspx'; });";
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", script, true);
                }
                else
                {
                    // Si no se cambió, mostrar alerta de error
                    string script = "Swal.fire('Error', 'No se pudo actualizar el estado de venta', 'error').then(function() { window.location.href = '31_ES_Salesseller.aspx'; });";
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", script, true);
                }
            }
            else
            {
                string script = "Swal.fire('Error', 'No se pudo utilizar el argumento', 'error').then(function() { window.location.href = '31_ES_Salesseller.aspx'; });";
                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", script, true);
            }
        }
