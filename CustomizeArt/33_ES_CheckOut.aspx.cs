...
{
    public partial class _33_ES_CheckOut : System.Web.UI.Page
    {
        ...
        protected void Buy_Click(object sender, EventArgs e)
        {
            if (txtcuenta.Text != "")
            {
                try
                {
                    ...
                    MySqlCommand comando = new MySqlCommand(String.Format("UPDATE usuarios SET Token_Compra = '{0}' WHERE Nombre_Usuario = '{1}'", randomCode, txtcuenta.Text), conexion2);
                    MySqlCommand evento = new MySqlCommand("CREATE EVENT eventonombre ON SCHEDULE AT CURRENT_TIMESTAMP + INTERVAL 2 MINUTE DO UPDATE usuarios SET Token_Compra = NULL", conexion2);
                    ...
                    const string subject = "Compra Confirmada";//Asunto del correo
                    string body = "Estimado " + nombrecliente + ",  en el presente correo le enviamos el token que solicitó. Muchas gracias por ser nuestro cliente. Token de Compra: " + randomCode + "";
                    ...
                    alerta.Text = "<script>Swal.fire('Correo enviado exitosamente', 'Token enviado', 'success');</script>";
                    Response.AddHeader("REFRESH", "3;URL=34_ES_Token-Compra.aspx");
                }
                catch
                {
