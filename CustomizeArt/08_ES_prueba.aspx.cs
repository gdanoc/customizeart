...
{
    public partial class _08_ES_prueba : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(productId))
                {
                    ...
                    {
                        string script = "Swal.fire('¡Inicia Sesión!', 'Necesitas tener una sesión iniciada para ver el carrito', 'info').then(function() { window.location.href = '01_ES_Login.aspx'; });";
        ...
        protected void Unnamed1_Click(object sender, EventArgs e)
        {
                ...
                if (CusImage.FileName != "" && impresion.SelectedValue != "Select face")
                {
                    if (DropDownList1.SelectedValue != "Selecciona la Talla" && colorRadioButtonList.SelectedValue != "")
                    {
                        ...
                        if (customizeId != 0)
                        {
                            ...
                            if (agregado != 0)
                            {
                                string script = "Swal.fire('¡Producto agregado al carrito!', '', 'success').then(function() { window.location.href = 'Default_Spanish.aspx'; });";
                                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", script, true);
                                Session["addedToCart"] = true;
                            }
                            else
                            {
                                string script = "Swal.fire('OOPS', 'No fue posible agregar el producto al carrito', 'error');";
                                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", script, true);
                            }
                        }
                        else
                    ...
                    {
                        string script = "Swal.fire('OOPS', 'No deje espacios en blanco', 'error')";
                        ScriptManager.RegisterStartupScript(this, GetType(), "showalert", script, true);
                    }
                }
                else
            ...
            {
                string script = "Swal.fire('OOPS', 'Id inválido', 'error').then(function() { window.location.href = 'Default_Spanish.aspx'; });";
                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", script, true);
            }
        }
    }
