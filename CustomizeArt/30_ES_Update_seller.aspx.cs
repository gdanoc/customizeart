...
{
    public partial class _30_ES_Update_seller : System.Web.UI.Page
    {
        ...
        protected void Fetch_product(int id)
        {
            try
            {
                ...
                {
                    alerta.Text = "<script>Swal.fire('Algo salió mal', 'Verifica el ID', 'error') </script>";
                }
            }
            catch (Exception exc)
            {
        ...
        protected void Unnamed2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(quantity))
                {
                    ...
                    if (productDetails.Id != 0)
                    {
                        ...
                        if (guardado != 0)
                        {
                            alerta.Text = "<script>Swal.fire('Productos agregados exitosamente', 'Gracias por escogernos', 'success'); </script>";
                            Load_Products();

                        }
                        else
                        {
                            alerta.Text = "<script>Swal.fire('Algo salió mal', 'Intentelo otra vez', 'error') </script>";
                        }
                    }
                    else
                    {
                        alerta.Text = "<script>Swal.fire('No se encontró el producto', 'Verifica el ID', 'error') </script>";
                    }
                }
                else
                {
            ...
            {
                alerta.Text = "<script>Swal.fire('Algo salió mal', 'Intentelo de nuevo', 'error') </script>";
            }
        }
