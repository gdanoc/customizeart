...
{
    public partial class _39_Sales_register : System.Web.UI.Page
    {
        ...
        protected void Load_Sales()
        {
            ...
            DataTable myTable = metodos.Get_Sales2(userId);

            // Agregar una nueva columna al DataTable para la imagen
            myTable.Columns.Add("Image", typeof(string));

            // Recorrer las filas para asignar el HTML de la imagen
            foreach (DataRow row in myTable.Rows)
            {
                string imagePath = row["ImagePath"].ToString();
                row["Image"] = "<img src='" + imagePath + "' width='100px' height='100px'/>";
            }

            // Remover la columna que muestra el nombre del archivo
            myTable.Columns.Remove("ImagePath");
        ...
        protected void Sale_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                Literal imgLiteral = (Literal)e.Row.FindControl("ImageLiteral");
                if (imgLiteral != null)
                {
                    imgLiteral.Text = DataBinder.Eval(e.Row.DataItem, "Image").ToString();
                }
